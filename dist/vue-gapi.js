...
var VueGAPI = {
  install: function (Vue$$1, clientConfig) {
    Vue$$1.gapiLoadClientPromise = null;
      ...
      gapiPromise.then(function (_) {
        ...
        if (!gapi.auth) {
          gapi.load('client:auth2', function () {
            Vue$$1.gapiLoadClientPromise = gapi.client
              .init(clientConfig)
              .then(function () {
                googleAuthService.authInstance = gapi.auth2.getAuthInstance();

                // Listen for sign-in state changes.
                gapi.auth2.getAuthInstance().isSignedIn.listen(Vue$$1.prototype.$getSignedInStatus);

                // Handle the initial sign-in state.
                Vue$$1.prototype.$getSignedInStatus(gapi.auth2.getAuthInstance().isSignedIn.get());
    ...
    Vue$$1.prototype.$getSignedInStatus = function (isSignedIn) {
      if (isSignedIn) {
        rootVue.$emit('signedIn', true);
      }
    };

    Vue$$1.prototype.$getGapiClient = function () {
      return new Promise(function (resolve, reject) {
        if (
          Vue$$1.gapiLoadClientPromise &&
          Vue$$1.gapiLoadClientPromise.status === 0
        ) {
          // promise is being executed
          resolve(Vue$$1.gapiLoadClientPromise);
        } else {
    ...
    Vue$$1.prototype.$login = function () {
      return Vue$$1.prototype.$getGapiClient().then(login)
    };

    Vue$$1.prototype.$logout = function () {
      return Vue$$1.prototype.$getGapiClient().then(logout)
    };

    Vue$$1.prototype.$isAuthenticated = isAuthenticated;

    Vue$$1.prototype.$getUserData = getUserData;
  }
};
